...
while True:
    ...
    trim3ModeloPC = trim2ModeloPC.strip("Product Name:") 
    ...
    direct_output4 = subprocess.check_output(
        'sudo lshw -class disk -class storage | grep '"product"'| uniq', shell=True)
    ModeloDR = direct_output4.decode('UTF-8')
    trim1ModeloDR = ModeloDR.strip()
    def func(value):
        return ''.join(value.splitlines())
    trim2ModeloDR = func(trim1ModeloDR)
    trim3ModeloDR = trim2ModeloDR.strip("Product:") 
    ...
    direct_output5 = subprocess.check_output(
        'sudo dmidecode --type 17 | grep '"Speed"' | uniq', shell=True)
