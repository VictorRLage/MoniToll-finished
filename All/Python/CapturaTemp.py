...
def teste():
    ...
    try:
        ...
        VerificarMetricas(var_leitura2, strNome, idTorre)
    except pyodbc.Error as err:
        crsr.rollback()
        print("Something went wrong: {}".format(err))
...
def VerificarMetricas(Leitura, strNome, idTorre):
    alertar = False
    frase = ''
    componente = ''
    if strNome == 'temperatura':
        componente = 'Temperatura CPU'
        if Leitura > int(m_cpu[0]) and Leitura < int(m_cpu[1]):
            frase = 'Alerta'
            alertar = True
        elif Leitura > int(m_cpu[1]) and Leitura < int(m_cpu[2]):
            frase = 'Perigo'
            alertar = True
        elif Leitura > int(m_cpu[2]):
            frase = 'Crítico'
            alertar = True
        else:
            print('Nenhum alerta emitido')
    if strNome == 'placa_mae':
        componente = 'Temperatura placa mãe'
        if Leitura > int(m_pm[0]) and Leitura < int(m_pm[1]):
            frase = 'Alerta'
            alertar = True
        elif Leitura > int(m_pm[1]) and Leitura < int(m_pm[2]):
            frase = 'Perigo'
            componente = 'RAM'
            alertar = True
        elif Leitura > int(m_pm[2]):
            frase = 'Crítico'
            alertar = True
        else:
            print('Nenhum alerta emitido')
    if strNome == 'desempenho':
        if Leitura > 3.5:
            frase = 'Crítico'
            alertar = True
